...
//static int8_t CDC_Itf_Transmit (uint8_t* pbuf);
...
//  /*##-3- Configure the TIM Base generation  #################################*/
  ...
  /* Start Channel1 */
  if(TIM_Base_Start_IT(&TimHandle) != HAL_OK)
  {
    /* Starting Error */
    Error_Handler();
  }
...
static int8_t CDC_Itf_Receive(uint8_t* pbuf, uint32_t *Len)
{
  //HAL_UART_Transmit_DMA(&UartHandle, pbuf, *Len);
...
//  {
//      APP_Tx_Buffer[APP_tx_ptr_head] = *(pbuf + i);
//      APP_tx_ptr_head++;
//
//      if(APP_tx_ptr_head == APP_TX_DATA_SIZE){
//              APP_tx_ptr_head = 0;
//      }
//
...
static void TIM_Config(void)
{
  ...
  */
  ...
  if(TIM_Base_Init(&TimHandle) != HAL_OK)
  {
    /* Initialization Error */
    Error_Handler();
  }
